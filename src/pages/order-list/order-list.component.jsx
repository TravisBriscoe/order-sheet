    ...
    <div className='order-sheet'>
      ...
      <div className='order-sheet-list'>
      {
        onOrderObj === null || onOrderObj.length <= 0  ?
          (<div className='order-sheet-list--noprods'><h3>No Products added to Order Sheet!</h3><h3>Please go to the product list and add products for the next order.</h3></div>)
        :
          onOrderObj.splice(0).map((el) => {
            return (
                <ul key={el[1].data.id}>
                  <li className='order-sheet-list-item'>
                    <div className='order-sheet-list-item--name'>{el[1].data.name}:</div>
                    <input className='order-sheet-list-item--quant' type='text' placeholder={el[1].data.value} onChange={(event) => props.onChangeOrderValue(event, el[1])} />
