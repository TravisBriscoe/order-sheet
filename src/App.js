	...
	onMenuSelect(sortedSelect) {
		this.setState({ sortCategory: sortedSelect }, () => {
			...
			if (sortCategory === "all") {
				...
				sortCategory === "findlays" ||
				sortCategory === "quattrocchis" ||
				sortCategory === "pigolive"
			) {
	...
	async deleteAllData(collection) {
		...
		if (window.confirm(`Are you sure you want to delete all ${text}?`)) {
			...
			if (collection === "users") {
				await userData()
					.then((snapshot) =>
						Object.entries(snapshot).filter((doc) => {
			...
			if (collection === products)
				this.setState({ products: "", sortedProds: "" }, () =>
					this.props.history.push("/manage/edit-products")
				);
			else if (collection === recipes)
				this.setState({ recipes: "" }, () => this.props.history.push("/manage/edit-recipes"));
			else if (collection === orderlist)
				this.setState({ onOrder: {} }, () => alert("Order-Sheet has been cleared!"));
		}
	...
	render() {
		...
		return (
			<div>
				{!loggedInUser ? (
					<div className="App">
						<LoginComponent setUserLoggedIn={this.setUserLoggedIn} {...this.state} />
					</div>
				) : (
					<div className="App">
						<Header
						...
						<Switch>
							...
							/>
							<Route
								path={`${process.env.PUBLIC_URL}/order-sheet`}
								render={(props) => (
									<OrderListPage
										{...props}
										onOrder={onOrder}
							...
							<Route
								...
								render={(props) => <RecipesPage {...props} recipes={recipes} />}
							/>
