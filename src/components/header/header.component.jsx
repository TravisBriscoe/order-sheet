		...
		return (
			<div className="header">
				<div className="header-title-user">
					{loggedInUser ? (
						<Link to={{ pathname: `${process.env.PUBLIC_URL}/manage`, state: { loggedInUser } }}>
							User: {loggedInUser}
						</Link>
					) : (
						<Link to={`/`}>Login</Link>
				...
				<h2 className="header-title-main">
					<Link to={`/`}>{title}</Link>
				</h2>
				<div className="header-title-recipes">
					<Link to={`${process.env.PUBLIC_URL}/recipes`}>Recipes</Link>
				</div>
			</div>
		);
	}
