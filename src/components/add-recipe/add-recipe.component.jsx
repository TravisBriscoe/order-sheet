		...
		return (
			<div className="add-recipe">
				<form className="add-recipe-form" onSubmit={this.onSave}>
					<input
						name="name"
						className="add-recipe-form-name"
					...
					{this.state.newRecipe.recipe.ingredients.map((x, i) => {
						return (
							<div key={i} className="add-recipe-form-ingredients-container">
								<input
									name="ingredients"
									className="add-recipe-form-ingredients-container-ingredient"
									placeholder="Ingredient:"
									onChange={(event) => this.onHandleIngreds(event, i)}
									value={this.state.newRecipe.recipe.ingredients[i] || ""}
								/>
								<button
									className="add-recipe-form-ingredients-container-ingredients--add_field"
									onClick={(event) => {
										event.preventDefault();
					...
					<textarea
						name="notes"
						className="add-recipe-form-notes"
						placeholder="Directions:"
						onChange={this.onHandleInput}
						value={this.state.newRecipe.recipe.notes || ""}
					/>
					{Object.entries(this.props.recipes).length <= 0 ? (
						<div className="add-recipe-form-actions">
							...
							<button className="add-recipe-form-action--clear" onClick={this.onClear}>
								Clear
							</button>
						</div>
					) : (
						<div className="add-recipe-form-actions">
							...
							<button className="add-recipe-form-action--cancel" onClick={this.onCancel}>
								Cancel
							</button>
						</div>
					)}
